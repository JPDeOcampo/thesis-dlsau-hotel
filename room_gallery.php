...
<html lang="en">
  ...
  <body>
	...
	<?php 
					...
					//$query5 = $conn->query("SELECT * FROM `transaction` WHERE `room_id` = '$_REQUEST[room_id]' && `status` = 'Pending Reservation'") or die(mysqli_error());
	                //$row = $query5->fetch_array();
					//$query6 = $conn->query("SELECT * FROM `transaction` WHERE `room_id` = '$_REQUEST[room_id]' && `status` = 'Pending Check In'") or die(mysqli_error());
  ...
  <div class="room-description"> 
    <div class="text p-4 p-xl-5 text-center"> 
      ...
      <p><span>Max:</span> <?php echo $fetch['max']?> Persons</p>
      <p><span>Bed:</span> <?php echo $fetch['bed']?></p>
      <p>Enjoy our service Quality.</p>
      <?php 
	  $id = $fetch['room_id'];
		echo '<button onclick="myFunction()">Book Now</button>';
	?>
    </div>
  </div>    
