  ...
  <body>
	    ...
	    <div class="container">
	      ...
	      <div class="collapse navbar-collapse" id="ftco-nav">
	        ...
	        <li class="nav-item active"><a href="book_room.php?date=<?php echo $_GET['date']?>&max=<?php echo $_GET['max']?>" class="nav-link" onclick="myFunctionRoom()">Room</a></li>	
    ...
    <section class="ftco-section bg-light ftco-no-pt ftco-no-pb">
 	...
 	<?php
					...
					$query5 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$chkdates' && `room_id` = '".$fetch4['room_id']."' && `status` = 'Pending Reservation'") or die(mysqli_error());
	                $row = $query5->fetch_array();
					$query6 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$chkdates' && `room_id` = '".$fetch4['room_id']."' && `status` = 'Pending Payment Reservation'") or die(mysqli_error());
	                $row2 = $query6->fetch_array();
			        $query7 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$chkdates' && `room_id` = '".$fetch4['room_id']."' && `status` = 'Pending Check In'") or die(mysqli_error());
      ...
      <div class="room-wrap d-md-flex"> <a class="img"><img src = "photo/<?php echo $fetch['photo']?>"  height = "100%" width = "100%"/> 
        ...
        <div class="half left-arrow d-flex align-items-center"> 
          <div class="text p-4 p-xl-5 text-center"> 
            ...
            <h3 class="mb-3"><a href="rooms.html"><?php echo $fetch2['room_type']?></a></h3>
            <ul class="list-accomodation">
			<?php
			if($max > $fetch4['max']){
              echo '<li style = "color:#ff0000;"><span>Max:</span> '.$fetch['max'].' Persons</li>';
			  }else{
			  echo '<li><span>Max:</span> '.$fetch['max'].' Persons</li>';
			  }
			  ?>
        ...
        if ($fetch = $row || $fetch = $row2 || $fetch = $row3){
            echo "<label style = 'color:#ff0000;'>**Room Is Not Available**</label>";
				echo '<p class="pt-1"><a href="room_gallery_book.php?date='.$chkdates.'&max='.$max.'&room_id='.$id.'" class="btn-custom px-3 py-2">View 
