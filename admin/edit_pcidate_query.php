...
<?php
	require_once 'connect.php';
		...
		//Check In
		$query = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkin' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Reservation'") or die(mysqli_error($conn));
		$row = $query->num_rows;
	    $query2 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkin' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Payment Reservation'") or die(mysqli_error($conn));
		$row2 = $query2->num_rows;
	    $query3 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkin' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Check In'") or die(mysqli_error($conn));
	    $row3 = $query3->num_rows;
		//$queryci = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkin' && `room_id` = '$roomid' && `status` = 'Check In'") or die(mysqli_error($conn));
	   // $rowci = $queryci->num_rows;
		//Check Out
		$query4 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkout' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Reservation'") or die(mysqli_error($conn));
		$row4 = $query4->num_rows;
	    $query5 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkout' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Payment Reservation'") or die(mysqli_error($conn));
		$row5 = $query5->num_rows;
	    $query6 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkout' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Check In'") or die(mysqli_error($conn));
	    ...
	    if($checkin < date('Y-m-d H:i:s')){
		         echo "<script type='text/javascript'>alert('Check In must be present date');history.go(-1);</script>";
		}elseif($checkout <= $checkin){	
				echo "<script type='text/javascript'>alert('Check Out must be present date');history.go(-1);</script>";
		}elseif($row >= 1 || $row2 >= 1 || $row3 >= 2){
					echo "<div class = 'modal-date' id='myModal'>
					      <div class='modal-contentdate'>
					            <span class='close'><a onclick='history.go(-1)'>&times;</a></span>
								<label style = 'color:#ff0000;'>Not Available Date For Check In</label><br />";
								//--------------------------------------------Pending Reservation--------------------------------------------------------------------------
		             $query_transpr = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkin' && `status` = 'Pending Reservation'") or die(mysqli_error($conn));
		             //----------------------------------------------Pending Payment Reservation----------------------------------------------------------------------
		           $query_transppr = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkin' && `status` = 'Pending Payment Reservation'") or die(mysqli_error($conn));
		          //------------------------------------------------------Pending Check In--------------------------------------------------------------
		         $query_transpc = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkin' && `status` = 'Pending Check In'") or die(mysqli_error($conn));
		         //----------------------------------------------------------------------------------------------------------------------
							//$q_date = $conn->query("SELECT * FROM `transaction` WHERE `status` = 'Pending Reservation'") or die(mysqli_error());
							$pr_datee = $conn->query("SELECT `checkin`, COUNT(checkin) FROM `transaction` WHERE `checkin` = '$checkin' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Reservation' GROUP BY `checkin` HAVING COUNT(checkin) >= 1;") or die(mysqli_error());
							$ppr_datee = $conn->query("SELECT `checkin`, COUNT(checkin) FROM `transaction` WHERE `checkin` = '$checkin' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Payment Reservation' GROUP BY `checkin` HAVING COUNT(checkin) >= 1;") or die(mysqli_error());
							$pci_datee = $conn->query("SELECT `checkin`, COUNT(checkin) FROM `transaction` WHERE `checkin` = '$checkin' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Check In' GROUP BY `checkin` HAVING COUNT(checkin) >= 1;") or die(mysqli_error());
							//$ci_datee = $conn->query("SELECT `checkin`, COUNT(checkin) FROM `transaction` WHERE `room_id` = '$roomid' && `status` = 'Check In' GROUP BY `checkin` HAVING COUNT(checkin) >= 1;") or die(mysqli_error());
					...
					echo "<div class = 'modal-date' id='myModal'>
		                 ...
		                 $query_transpc = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkout' && `status` = 'Pending Check In'") or die(mysqli_error($conn));
		                  //----------------------------------------------------------------------------------------------------------------------
							//$q_date = $conn->query("SELECT * FROM `transaction` WHERE `status` = 'Pending Reservation'") or die(mysqli_error());
							$pr_datee = $conn->query("SELECT `checkin`, COUNT(checkin) FROM `transaction` WHERE `checkin` = '$checkout' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Reservation' GROUP BY `checkin` HAVING COUNT(checkin) >= 1;") or die(mysqli_error());
							$ppr_datee = $conn->query("SELECT `checkin`, COUNT(checkin) FROM `transaction` WHERE `checkin` = '$checkout' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Payment Reservation' GROUP BY `checkin` HAVING COUNT(checkin) >= 1;") or die(mysqli_error());
							$pci_datee = $conn->query("SELECT `checkin`, COUNT(checkin) FROM `transaction` WHERE `checkin` = '$checkout' && `room_id` = '".$row_transid['room_id']."' && `status` = 'Pending Check In' GROUP BY `checkin` HAVING COUNT(checkin) >= 1;") or die(mysqli_error());
							//$pci_date = $conn->query("SELECT * FROM `transaction` WHERE `status` = 'Pending Check In'") or die(mysqli_error($conn));
							echo "___________________________________________";
							echo "<br><label><mark>Pending Reservation:<mark></label></br>";
							if($row4 == 0){
		...
		echo "<script>
	alert('Successfully Change a Guest Date Details!');history.go(-1);
