            ...
            <div class="container-fluid">
                ...
                <!-- /# row -->
                 ...
                 <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">Pending Reservation Details</h4>
                                <?php
			    $q_pr = $conn->query("SELECT COUNT(*) as total FROM `transaction` WHERE `status` = 'Pending Reservation'") or die(mysqli_error());
				$f_pr = $q_pr->fetch_array();
				
			?>
                 <div class="table-responsive m-t-40">
				 <a class = "btn btn-success disabled"><span class = "badge"><?php echo $f_pr['total']?></span> Pendings Reservation</a>
				...
				<br />
				   ...
				   <table id="example24" class="newTable" cellspacing="0">
					...
					<tbody>
						<?php
							$sql = "SELECT * FROM `guest` NATURAL JOIN `transaction` WHERE `status` = 'Pending Reservation'";
							...
							$reservation_total = $row2['price'] * $days;		
						?>
						<tr>
						    <td><?php echo $i;?></td>
							...
							<td>PHP<?php echo $reservation_total?>.00</td>
							<td><center><a class = "btn btn-success" href = "confirm_reservation.php?transaction_id=<?php echo $row['transaction_id']?>"><i class = "glyphicon glyphicon-check"></i> Confirm</a> <a class = "btn btn-warning" onclick = "confirmationDelete(); return false;" href = "cancel_pendingreservation.php?transaction_id=<?php echo $row['transaction_id']?>&guest_id=<?php echo $row['guest_id']?>"><i class = "glyphicon glyphicon-trash"></i> Cancel</a> <a class = "btn btn-danger" onclick = "confirmationDelete(); return false;" href = "decline_reservation.php?transaction_id=<?php echo $row['transaction_id']?>&guest_id=<?php echo $row['guest_id']?>"><i class = "glyphicon glyphicon-trash"></i> Decline</a></td>
						</tr>
						<?php $i++;?>
						<?php
							}
