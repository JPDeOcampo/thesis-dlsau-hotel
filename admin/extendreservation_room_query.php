...
<?php
	require_once 'connect.php';
		...
		//-------------------------------------------------------Check In--------------------------------------------------------------
		$query_transci = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '".$row_transid['checkin']."' && `status` = 'Check In'") or die(mysqli_error($conn));
		
		
		$query_transci_row = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '".$row_transid['checkin']."' && `status` = 'Check In'") or die(mysqli_error($conn));
		$row_transci_row = $query_transci_row->num_rows;
		...
		//Check Out
		$query4 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '".$row_transid['checkin']."' && `room_id` = '$room_type' && `status` = 'Pending Reservation'") or die(mysqli_error($conn));
		$row4 = $query4->num_rows;
	    $query5 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '".$row_transid['checkin']."' && `room_id` = '$room_type' && `status` = 'Pending Payment Reservation'") or die(mysqli_error($conn));
		$row5 = $query5->num_rows;
	    $query6 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '".$row_transid['checkin']."' && `room_id` = '$room_type' && `status` = 'Pending Check In'") or die(mysqli_error($conn));
	    $row6 = $query6->num_rows;
		$query7 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '".$row_transid['checkin']."' && `room_id` = '$room_type' && `status` = 'Check In'") or die(mysqli_error($conn));
	    $row7 = $query7->num_rows;
		        ...
		        echo "<script type='text/javascript'>alert('The hotel only allow to upgrade room');history.go(-1);</script>";
		...
		}elseif($row4 >= 1 || $row5 >= 1 || $row6 >= 1 || $row7 >= 2){
					echo "<div class = 'modal-date' id='myModal'>
					      <div class='modal-contentdate'>
							...
							$ci_datee = $conn->query("SELECT `checkin`, COUNT(checkin) FROM `transaction` WHERE `checkin` = '".$row_transid['checkin']."' && `room_id` = '$room_type' && `status` = 'Check In' GROUP BY `checkin` HAVING COUNT(checkin) >= 1;") or die(mysqli_error());
							...
							//--------------------------------------Check In-------------------------------------------------------
						    echo "___________________________________________";
							echo "<br><label><mark>Current Check In:</mark></label></br>";
							if($row_transci_row == 0){
							 echo " ";
							
							}else{
							while($ci_date = $ci_datee->fetch_array()){
							
							echo "
								    <ul>
										
										   <label class = 'alert-danger'><span>**</span>".date("M d, Y", strtotime($ci_date['checkin']."+7HOURS")).": '".$row44['room_type']."'<span>**</span></label>		
										
									</ul>";
							
							}
						 while($row_transci = $query_transci->fetch_array()){
						$query_transci1 = $conn->query("SELECT * FROM `room` WHERE `room_id` = '".$row_transci['room_id']."'") or die(mysqli_error($conn));
	                    $row_transci1 = $query_transci1->fetch_array();
		
		                $query_transci2 = $conn->query("SELECT * FROM `room_type` WHERE `roomtype_id` = '".$row_transci1['room_type']."'") or die(mysqli_error($conn));
	                    $row_transci2 = $query_transci2->fetch_array();
							echo "
								    <ul>
										<li>
										   <label class = 'alert-danger'>".date("M d, Y", strtotime($row_transci['checkin']."+7HOURS")).": '".$row_transci2['room_type']."'</label>		
										</li>
									</ul>";
							}
							}
